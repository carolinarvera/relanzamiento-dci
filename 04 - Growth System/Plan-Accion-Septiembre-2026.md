...
- **Corregir el tag GA4 de AXXIS — y confirmado que sigue activo en agosto.** Tracking mezclado con Diners detectado en julio (~22-24 jul), y ahora confirmado de nuevo: el GA4 de Diners de agosto muestra "Revista AXXIS" entre sus páginas más vistas (3,695 vistas). No se corrigió. Ningún reporte de GA4 de ninguna de las dos marcas es válido hasta la corrección y un ciclo limpio de medición.
...
- **Causa técnica probable encontrada para el embudo "roto" de Diners: falta cross-domain tracking con ePayco.** GA4 1-25 ago: 104,761 sesiones, solo 3 eventos clave (0.003%), pero ~$41,472 en ingresos registrados en el mismo periodo — paradoja que se explica porque `new-checkout.epayco.co` aparece como fuente de referencia **externa** en vez de continuación de sesión. GA4 pierde el hilo cuando el usuario pasa al checkout de ePayco, así que el evento de compra completada no se ata a la sesión de origen. **Acción:** en GA4 → Administrador → Configuración de flujo de datos → Medición entre dominios, agregar `epayco.co` a la lista de dominios vinculados. Antes de esto, verificar si AXXIS tiene el mismo gap (su pasarela también es e-Payco según memoria del vault) — podría ser la misma causa raíz para ambas marcas. Consistente con otro dato del panorámico: `submit_lead_form` registró 43 eventos contra solo 3 compras — una caída del 93% que probablemente incluye conversiones reales perdidas por este mismo gap de tracking.
