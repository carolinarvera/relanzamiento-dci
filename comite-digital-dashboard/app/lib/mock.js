...
export function mockEmailTraffic(range) {
  ...
  const build = (brand, scale) => {
    ...
    return {
      ...
      articles: [['/arquitectura/casa-en-la-montana/', 'Casa en la montaña: un refugio que dialoga con el paisaje', 'arquitectura', 0.09], ['/diseno/interiorismo-objetos-bazar/', 'Interiorismo y objetos de diseño para el hogar', 'diseno', 0.07], ['/decoracion/apartamento-escandinavo/', 'Apartamento de inspiración escandinava', 'decoracion', 0.05], ['/arquitectura/museo-de-arte-moderno/', 'El museo que reinventa el arte moderno', 'arquitectura', 0.04]].map(([path, title, topic, f], k) => ({ path, title, topic, date: `2026-08-${String(3 + k * 5).padStart(2, '0')}`, views: Math.round(views * f), entries: Math.round(views * f * 0.6) })),
      otherPages: [['/marketplace/', 0.06], ['/', 0.05]].map(([path, f]) => ({ path, views: Math.round(views * f) })),
      articleTotal: Math.round(views * 0.7),
